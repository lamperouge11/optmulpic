...
func write_Ciphertext(cont *context, ct_conv *ckks.Ciphertext, log_slots int,name string){
		vals_tmp1 := cont.encoder.Decode(cont.decryptor.DecryptNew(ct_conv), log_slots)
			
		res1 := make([]float64, len(vals_tmp1))
		
		for i := 0; i < len(vals_tmp1); i++ {
			res1[i] = real(vals_tmp1[i])			
		}		
		writeTxt(name, res1)

}

func write_Ciphertext_image(cont *context, ct_conv *ckks.Ciphertext, log_slots int,name string){
		vals_tmp1 := cont.encoder.Decode(cont.decryptor.DecryptNew(ct_conv), log_slots)
			
		res1 := make([]float64, len(vals_tmp1))
		
		for i := 0; i < len(vals_tmp1); i++ {
			res1[i] = imag(vals_tmp1[i])			
		}		
		writeTxt(name, res1)
...
func testConv_BL_in(real_batch, in_wid, ker_wid, total_test_num int, boot bool) {
	...
	for test_iter := 0; test_iter < total_test_num; test_iter++ {
		...
		if boot {
			...
			//real_out = make([]float64, raw_in_wid*raw_in_wid*in_batch)
		} else {
		...
		for i := 0; i < raw_in_wid; i++ {
			for j := 0; j < raw_in_wid; j++ {
				for b := 0; b < real_batch/2; b++ {
					...
					//pad_input1[b+j*real_batch/2+i*real_batch/2*in_wid] = 1
					//pad_input2[b+j*real_batch/2+i*real_batch/2*in_wid] = 1
				}
			}
		}
				...
				for k := 0; k < ker_size; k++ {
					for i := 0; i < real_batch/2; i++ { // in
						for j := 0; j < real_batch/2; j++ { // out
							...
							//ker_in_sep[out][in][k*real_batch*real_batch/4+i*real_batch/2+j] = 1
		...
		start := time.Now()
		//fmt.Printf("input1_rs = \n", real(input1_rs[0]),imag(input1_rs[0]))
		//cont.ECD_LV = 19
		//fmt.Println("cont.ECD_LV = ", cont.ECD_LV)
		...
		//fmt.Println("before Conv_BN_BL_test: LV = ", ct_input1.Level())
		for pos := 0; pos < 2; pos++ {
		...
		//fmt.Println("after Conv_BN_BL_test: LV = ", ct_res[0].Level())
		fmt.Printf("Evaluation total done in %s \n", time.Since(start_eval))
		
		
		/*
		pl_scale := ckks.NewPlaintext(cont.params, ct_res[0].Level(), math.Pow(2, 30)*float64(cont.params.Q()[14])*float64(cont.params.Q()[13])/ct_res[0].Scale)
		...
		cont.evaluator.Mul(ct_res[0], pl_scale, ct_res[0])
			
		cont.evaluator.Rescale(ct_res[0], cont.params.Scale(), ct_res[0])
		
		
		fmt.Println("after pl_scale: LV = ", ct_res[0].Level(), " Scale = ", ct_res[0].Scale,math.Log2(ct_res[0].Scale))
		
		ct_res[0] = evalReLU(cont.params, cont.evaluator, ct_res[0], alpha)
		cont.evaluator.MulByPow2(ct_res[0], int(pow), ct_res[0])
		cont.evaluator.SetScale(ct_res[0], cont.params.Scale())
		
		
		
		//fmt.Println("after relu: LV = ", ct_res[0].Level(), " Scale = ", ct_res[0].Scale,math.Log2(ct_res[0].Scale))
		...
		*/
		...
		if boot {
			...
			ct_boot := cont.btp.Bootstrapp(ct_res[0])
			cont.evaluator.MulByPow2(ct_boot, int(17 + 2), ct_boot)
			vals_tmp2 := cont.encoder.Decode(cont.decryptor.DecryptNew(ct_boot), log_slots)
			
			for i := 0; i < 100; i++ {
			
				fmt.Printf("vals_tmp1 = %6.10f %6.10f %6.10f\n", real(vals_tmp1[i]),real(vals_tmp2[i]), real(vals_tmp1[i]) - real(vals_tmp2[i]))
			...
			cont.evaluator.Mul(ct_res[0], pl_scale, ct_res[0])			
			cont.evaluator.Rescale(ct_res[0], cont.params.Scale(), ct_res[0])			
			cont.evaluator.MulByPow2(ct_res[0], int(15), ct_res[0])
			
			cont.evaluator.Mul(ct_res[1], pl_scale, ct_res[1])			
			cont.evaluator.Rescale(ct_res[1], cont.params.Scale(), ct_res[1])			
			cont.evaluator.MulByPow2(ct_res[1], int(15), ct_res[1])	
			...
			for pos := 0; pos < 2; pos++ {
				...
				cont.evaluator.MulByPow2(ct_res[pos], int(18-pow-1), ct_res[pos])
		...
		/*
		fmt.Println("after relu: LV = ", ct_res[0].Level(), " Scale = ", ct_res[0].Scale,math.Log2(ct_res[0].Scale))
		fmt.Println("after relu: LV = ", ct_res[1].Level(), " Scale = ", ct_res[1].Scale,math.Log2(ct_res[1].Scale))
		
		
		fmt.Printf("Done all in %s \n", time.Since(start_xc))
		// fmt.Println()
		...
		ct_res[0] = evalConv_BN_BL_test(cont, ct_res[0], ker_in_sep[0][0], bn_a_sep[0], bn_b_sep[0], in_wid, ker_wid, real_batch/2, real_batch/2, 0, 1, pad, false, false)
		fmt.Println("before Boot: LV = ", ct_res[0].Level(), " Scale = ", ct_res[0].Scale,math.Log2(ct_res[0].Scale))
		write_Ciphertext(cont,ct_res[0],log_slots,"BL_evalConv2")
		
		
		ct_res[0] = cont.btp.Bootstrapp(ct_res[0])
		write_Ciphertext(cont,ct_res[0],log_slots,"BL_Bootstrapp2")
		fmt.Println("after Boot: LV = ", ct_res[0].Level(), " Scale = ", ct_res[0].Scale,math.Log2(ct_res[0].Scale))
		...
		write_Ciphertext(cont,ct_res[0],log_slots,"BL_pl_scale2")
		fmt.Println("after Boot: LV = ", ct_res[0].Level(), " Scale = ", ct_res[0].Scale,math.Log2(ct_res[0].Scale))
		
		
		ct_res[0] = evalReLU(cont.params, cont.evaluator, ct_res[0], alpha)
		cont.evaluator.MulByPow2(ct_res[0], int(18-pow), ct_res[0])
		cont.evaluator.SetScale(ct_res[0], cont.params.Scale())
		
		//cont.evaluator.SetScale(ct_res[0], cont.params.Scale())
		write_Ciphertext(cont,ct_res[0],log_slots,"BL_evalReLU2")
		fmt.Println("after evalReLU: LV = ", ct_res[0].Level(), " Scale = ", ct_res[0].Scale,math.Log2(ct_res[0].Scale))
