	...
	for i := 0; i < in_wid; i++ {
		for j := 0; j < in_wid; j++ {
			for k := 0; k < batch; k++ {
				...
				//out[i*in_wid+j+k*in_wid*in_wid] = complex(input[l], input[l]) //xuchao
				l++
			}
		}
	}
...
func reshape_input_BL_xc(input []float64, in_wid int) (out []complex128) {
	out = make([]complex128, len(input))
	batch := len(input) / (in_wid * in_wid)
	l := 0
...
func postConv_BL_xc(param ckks.Parameters, encoder ckks.Encoder, evaluator ckks.Evaluator, ct_in_rots []*ckks.Ciphertext, in_wid, ker_wid, rot, pad, gap int, max_ker_rs [][][][]float64) (ct_out *ckks.Ciphertext) {
	...
	for i := 0; i < ker_wid; i++ {
		for j := 0; j < ker_wid; j++ {
			...
			nm := int(param.Slots()/gap)
			for g := 0; g < gap; g++ {	
				for aaa := 0; aaa < nm; aaa++ {		
					postKer[aaa + nm * g] = postKer[aaa]
...
// eval Convolution for the part of output: need to sum this up with rotations
func postConv_BL_xuchao(param ckks.Parameters, encoder ckks.Encoder, evaluator ckks.Evaluator, ct_in_rots []*ckks.Ciphertext, in_wid, ker_wid, rot, pad int, max_ker_rs, max_ker_rs_1 [][][][]float64) (ct_out *ckks.Ciphertext) {
	...
	for i := 0; i < ker_wid; i++ {
		for j := 0; j < ker_wid; j++ {
			for k := 0; k < max_batch; k++ {
				for ki := 0; ki < (in_wid - pad); ki++ { // position of input
					for kj := 0; kj < (in_wid - pad); kj++ {
						postKer[k*in_wid*in_wid+ki*in_wid+kj] = complex(max_ker_rs[i][j][k][(k-rot+max_batch)%max_batch], max_ker_rs_1[i][j][k][(k-rot+max_batch)%max_batch])
						if (((ki + i - (ker_wid / 2)) < 0) || ((ki + i - (ker_wid / 2)) >= (in_wid - pad))) || (((kj + j - (ker_wid / 2)) < 0) || ((kj + j - (ker_wid / 2)) >= (in_wid - pad))) {
							postKer[k*in_wid*in_wid+ki*in_wid+kj] = complex(0, 0)
						}
					}
...
func encode_ker_final(ker_in [][]float64, pos, i, in_wid, in_batch, ker_wid int) []float64 {
	...
	for j := 0; j < in_batch; j++ { // j-th input (batch)
		for k := 0; k < k_sz; k++ {
			...
			index := (in_wid*(k/ker_wid)+k%ker_wid)*in_batch+j
			/*
			if index == 0 {
				index = vec_size - 1
			} else {
			...
			*/
			output[index] = ker_in[i][(in_batch-1-j)*k_sz+(k_sz-1-k)+bias] // * scale;			
...
func pack_ctxts(pack_eval ckks.Evaluator, ctxts_in []*ckks.Ciphertext, max_cnum, real_cnum int, idx []*ckks.Plaintext, params ckks.Parameters) (ctxt *ckks.Ciphertext) {
	...
	fmt.Println("pack_ctxts:",  step , norm)
	for step >= norm {
...
func ext_double_ctxt(eval ckks.Evaluator, encoder ckks.Encoder, input *ckks.Ciphertext, m_idx, r_idx map[int][]int, params ckks.Parameters) (result *ckks.Ciphertext) {
	...
	for rot, elt := range m_idx {
		...
		//fmt.Println("ext_double_ctxt 1111111111 m_idx:", rot, len(elt))
	...
	for rot, elt := range r_idx {
		...
		//fmt.Println("ext_double_ctxt 22222222222 r_idx:", rot, len(elt))
		plain_tmp := ckks.NewPlaintext(params, input.Level(), sqr_sc)
		encoder.EncodeNTT(plain_tmp, tmp, params.LogSlots())
		if st {
			result = eval.RotateNew(eval.MulNew(mid_result, plain_tmp), rot)
...
func ext_double_ctxt_xuchao(cont *context, eval ckks.Evaluator, encoder ckks.Encoder, input *ckks.Ciphertext, m_idx, r_idx map[int][]int, params ckks.Parameters, ul, in_wid, step int) (result *ckks.Ciphertext) {
	st := true
	sqr_sc := math.Sqrt(float64(params.Q()[input.Level()]))
	...
	//for i := 0; i < 1; i++ {	
		...
		for i := range elt {
			if elt[i] == 1 {
				idx := reverseBits(uint32(i), 9)
				if ul == 1 && ((int)(idx)) >= (step * in_wid / 2 * (in_wid / 2 / 2 - 1)) {
					//fmt.Println("ext_double_ctxt 1111111111 m_idx:", rot, idx)
					continue
				}			
			}	
			tmp[i] = complex(float64(elt[i]), 0)
		}
		//tmp[0] = complex(float64(1.0), 0)
	...
	//mid_result = eval.RotateNew(mid_result, 2)
	...
	for i := 0; i < params.Slots(); i++ {
		...
		tmp := make([]complex128, params.Slots())
		//fmt.Println("ext_double_ctxt 222222 r_idx:", len(elt), len(tmp),params.Slots())
		tmp[1] = complex(float64(1.0), 0)
		
		for i := range elt {
			
			tmp[i] = complex(float64(elt[i]), 0)
			if i == 1 {			
				fmt.Println("ext_double_ctxt 222222 r_idx:", rot, i, len(elt))
			}
		}
		...
		encoder.EncodeNTT(plain_tmp, tmp, params.LogSlots())
		if st {
			result = eval.RotateNew(eval.MulNew(mid_result, plain_tmp), i)
			st = false
		} else {
			ctxt_tmp := eval.RotateNew(eval.MulNew(mid_result, plain_tmp), i)
			eval.Add(result, ctxt_tmp, result)
		}
		//break
	}
	*/
	...
	eval.Rescale(mid_result, params.Scale(), mid_result)
...
func keep_ctxt(params ckks.Parameters, eval ckks.Evaluator, encoder ckks.Encoder, input *ckks.Ciphertext, idx []int) (result *ckks.Ciphertext) {
	...
	for i := range idx {
		...
		*/
		//tmp[i] = complex(float64(1.0), 0)
	}
	fmt.Println("keep_ctxt: params.Slots()", params.Slots())
	// plain_tmp := encoder.EncodeNTTAtLvlNew(input.Level(), tmp, params.LogSlots())
	plain_tmp := ckks.NewPlaintext(params, input.Level(), float64(params.Q()[input.Level()]))
	encoder.EncodeNTT(plain_tmp, tmp, params.LogSlots())
...
func keep_ctxt_xuchao(params ckks.Parameters, eval ckks.Evaluator, encoder ckks.Encoder, input *ckks.Ciphertext, idx []int) (result *ckks.Ciphertext) {
	tmp := make([]complex128, params.Slots())
	//c := 0
	for i := range idx {
...
func keep_ctxt_xuchao111(params ckks.Parameters, eval ckks.Evaluator, encoder ckks.Encoder, input *ckks.Ciphertext, idx, id_reverse []int, max_pic int) (result []*ckks.Ciphertext) {
	tmp := make([]complex128, params.Slots())
	tmp111 := make([]float64, params.Slots())
	//c := 0
	for i := range idx {
		
		tmp[i] = complex(float64(idx[i]), 0)
		//tmp[i] = complex(float64(1.0), 0)
		tmp111[i] = float64(idx[i])		
	}
	tmpxc := make([][]complex128, max_pic)
	
	writeTxt("idx",tmp111)
	
	for i := 0; i < max_pic; i++ {
		tmpxc[i] = make([]complex128, params.Slots())
		//tmpxc[i] = tmp
	}
	...
	result = make([]*ckks.Ciphertext, max_pic)
	
	// plain_tmp := encoder.EncodeNTTAtLvlNew(input.Level(), tmp, params.LogSlots())
	for i := 0; i < max_pic; i++ {
		plain_tmp := ckks.NewPlaintext(params, input.Level(), float64(params.Q()[input.Level()]))
		encoder.EncodeNTT(plain_tmp, tmpxc[i], params.LogSlots())

		result[i] = eval.MulNew(input, plain_tmp)
	
		eval.Rescale(result[i], params.Scale(), result[i])
	}
	return result
}
...
func evalReLU(params ckks.Parameters, evaluator ckks.Evaluator, ctxt_in *ckks.Ciphertext, alpha float64) (ctxt_out *ckks.Ciphertext) {
	...
	//fmt.Println("Degree: ", coeffsReLU.Degree())
	...
	//fmt.Println("Degree: ", coeffsReLU2.Degree())
	...
	//fmt.Println("relu 4444444444444: LV = ", ctxt_out.Level())
	evaluator.Mul(ctxt_out, ctxt_in, ctxt_out)
	evaluator.Relinearize(ctxt_out, ctxt_out)
	//fmt.Println("relu 5555555555555: LV = ", ctxt_out.Level())
	return
}
...
func prep_Ker(params ckks.Parameters, encoder ckks.Encoder, ker_in, BN_a []float64, in_wid, ker_wid, real_ib, real_ob, norm, ECD_LV, pos int, trans bool) (pl_ker []*ckks.Plaintext) {
	...
	for i := 0; i < real_ob; i++ {
		for j := 0; j < real_ib; j++ {
			...
			/*
				if norm*j*ker_size+k-1 < 0 {
					max_ker_rs[norm*i][max_bat*ker_size - 1] = ker_rs[i][j*ker_size+k]
				} else {
					max_ker_rs[norm*i][norm*j*ker_size+k - 1] = ker_rs[i][j*ker_size+k]
				}
				*/
				max_ker_rs[norm*i][norm*j*ker_size+k] = ker_rs[i][j*ker_size+k]
				//max_ker_rs[norm*i][norm*j*ker_size+k] = 1.0 //xuchao
			}
		}
		//fmt.Println("max_ker_rs = ", i , norm*i, max_ker_rs[norm*i])
	}
...
func prep_Ker_xxx(params ckks.Parameters, encoder ckks.Encoder, ker_in, BN_a []float64, in_wid, ker_wid, real_ib, real_ob, norm, move, ECD_LV, pos int, trans bool) (pl_ker []*ckks.Plaintext) {
	max_bat := params.N() / (in_wid * in_wid)
	ker_size := ker_wid * ker_wid
	ker_rs := reshape_ker(ker_in, ker_size, real_ob, trans) // ker1[i][j] = j-th kernel for i-th output
	...
	for i := 0; i < real_ob; i++ {
		for j := 0; j < real_ib; j++ {
			for k := 0; k < ker_size; k++ {
				...
				if norm*j*ker_size+k - move < 0 {
					max_ker_rs[norm*i][max_bat*ker_size - move] = ker_rs[i][j*ker_size+k]
				} else {
					max_ker_rs[norm*i][norm*j*ker_size+k - move] = ker_rs[i][j*ker_size+k]
				}
...
func prep_Ker_xuchao(params ckks.Parameters, encoder ckks.Encoder, ker_in, BN_a []float64, in_wid, ker_wid, real_ib, real_ob, norm, ECD_LV, pos, split_batch int, trans bool) (pl_ker [][][]*ckks.Plaintext) {
	
	if real_ib < split_batch || real_ob < split_batch {
		panic("real_ib and real_ib should be greater equal split_batch")
	}
	
	max_bat := params.N() / (in_wid * in_wid)
	fmt.Println("max_bat = ", max_bat)
	ker_size := ker_wid * ker_wid
	ker_rs := reshape_ker(ker_in, ker_size, real_ob, trans) // ker1[i][j] = j-th kernel for i-th output
	...
	max_ker_rs_split := make([][][][]float64, split_batch) // add xuchao
	
	for a := 0; a < split_batch; a++ {
		max_ker_rs_split[a] = make([][][]float64, split_batch)
		for b := 0; b < split_batch; b++ {
			max_ker_rs_split[a][b] = make([][]float64, max_bat)
			for i := 0; i < max_bat; i++ {
				max_ker_rs_split[a][b][i] = make([]float64, max_bat*ker_size)
			}
		}
	}
	...
	for i := 0; i < real_ob; i++ {
		for j := 0; j < real_ib; j++ {
			...
			for k := 0; k < ker_size; k++ {
				a_delta := real_ob/split_batch
				a_idx := (int)(i/a_delta)
				
				b_delta := real_ib/split_batch
				b_idx := (int)(j/b_delta)
				max_ker_rs_split[a_idx][b_idx][(i % a_delta) * norm][norm * (j % b_delta) * ker_size+k] = ker_rs[i][j*ker_size+k]
				//max_ker_rs_split[a_idx][b_idx][(i % a_delta) * norm][norm * (j % b_delta) * ker_size+k] = 1.0
			}
		}
	}

	// test max_ker_rs_split by xuchao
	/*
	max_ker_rs := make([][]float64, max_bat * split_batch) // overloading ker_rs to the case with max_batch
	for i := 0; i < max_bat * split_batch; i++ {
		max_ker_rs[i] = make([]float64, max_bat * ker_size * split_batch)
	}
	...
	for a := 0; a < split_batch; a++ { // out 
		for b := 0; b < split_batch; b++ { // in
			for i := 0; i < len(max_ker_rs_split[a][b]); i++ {
				for j := 0; j < len(max_ker_rs_split[a][b][i]); j++ {
				
					value := max_ker_rs_split[a][b][i][j]
					a_delta := real_ob/split_batch
					b_delta := real_ib/split_batch
					
					if value != max_ker_rs[a * a_delta + i][b * b_delta * norm * ker_size + j] {
						fmt.Println("xuchao ",value, max_ker_rs[a * a_delta + i][ b * b_delta + j ])
	...
	*/
	...
	pl_ker_split := make([][][]*ckks.Plaintext, split_batch)
	for a := 0; a < split_batch; a++ {
		pl_ker_split[a] = make([][]*ckks.Plaintext, split_batch)
		for b := 0; b < split_batch; b++ {
			pl_ker_split[a][b] = make([]*ckks.Plaintext, max_bat)
		}		
	}
	...
	for a := 0; a < split_batch; a++ {
		for b := 0; b < split_batch; b++ {
			for i := 0; i < max_bat; i++ {
				pl_ker_split[a][b][i] = ckks.NewPlaintext(params, ECD_LV, params.Scale())
				encoder.EncodeCoeffs(encode_ker_final(max_ker_rs_split[a][b], pos, i, in_wid, max_bat, ker_wid), pl_ker_split[a][b][i])
				encoder.ToNTT(pl_ker_split[a][b][i])
			}
		}		
	}
...
func prep_Ker_renset_xuchao(params ckks.Parameters, encoder ckks.Encoder, ker_in, BN_a []float64, in_wid, ker_wid, real_ib, real_ob, norm, ECD_LV, pos, split_batch int, trans bool) (pl_ker [][][]*ckks.Plaintext) {
	/*
	if real_ib < split_batch || real_ob < split_batch {
		panic("real_ib and real_ib should be greater equal split_batch")
	}
	*/
	...
	max_ker_rs_split := make([][][][]float64, ob_split) // add xuchao
	
	for a := 0; a < ob_split; a++ {
		max_ker_rs_split[a] = make([][][]float64, ib_split)
		for b := 0; b < ib_split; b++ {
			max_ker_rs_split[a][b] = make([][]float64, max_bat)
			for i := 0; i < max_bat; i++ {
				max_ker_rs_split[a][b][i] = make([]float64, max_bat*ker_size)
			}
	...
	for i := 0; i < real_ob; i++ {
		for j := 0; j < real_ib; j++ {
			for k := 0; k < ker_size; k++ {
				a_delta := real_ob/ob_split
				a_idx := (int)(i/a_delta)
				
				b_delta := real_ib/ib_split
				b_idx := (int)(j/b_delta)				
				max_ker_rs_split[a_idx][b_idx][(i % a_delta) * norm][norm * (j % b_delta) * ker_size+k] = ker_rs[i][j*ker_size+k]
	...
	pl_ker_split := make([][][]*ckks.Plaintext, ob_split)
	for a := 0; a < ob_split; a++ {
		pl_ker_split[a] = make([][]*ckks.Plaintext, ib_split)
		for b := 0; b < ib_split; b++ {
			pl_ker_split[a][b] = make([]*ckks.Plaintext, max_bat)
		}		
	}
