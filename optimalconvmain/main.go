...
func newContext(logN, ker_wid int, in_wids, kp_wids []int, boot bool, kind string) *context {
	...
	btpParams := ckks.DefaultBootstrapParams[6] // xuchao btpParams := ckks.DefaultBootstrapParams[6]
	...
	if logN < 16 {
		btpParams.LogN = logN
		btpParams.LogSlots = logN - 1
			
	}
	//fmt.Printf("newContext newContext newContext",logN)
	...
	if (kind == "Resnet_crop_sparse") || (kind == "Resnet_crop_sparse_wide2") || (kind == "Resnet_crop_sparse_wide3") || (kind == "Imagenet_sparse") || (kind == "Conv") { // generate 2 more params for sparse boot (logSlots, -1, -2)
		if (kind == "Conv") {
			btpParams.LogN = 10
		} else {
			btpParams.LogN = 16
	...
	if cont.params.N() != cont.N {
		fmt.Println("Set Boot logN to", logN,cont.params.N(), cont.N)
		panic("Boot N != N")
	}
	// Gen rotations
	var rotations []int
	...
	cont.idx_reverse = make([]int, 32768)
	for it := 0; it < 32768; it++ {
		id := reverseBits(uint32(it),15)
		cont.idx_reverse[it] = int(id)
	}
	...
	case "BL_Conv":
		...
		for aa := 0; aa < 31; aa++{
				for bb := 0; bb < 31; bb++{
					if aa % 2 == 0 && bb % 2 == 0 {
						mv := aa*32*32 + bb*32 - (aa*32*32/2 + bb*32/2)
						if mv > 32*32*32 {
							fmt.Println("mvmvmvmv = ",mv)
						}
						rotations = append(rotations, mv)
					}
				}
		}
		...
		for aa := 0; aa < 15; aa++{
				for bb := 0; bb < 15; bb++{
					if aa % 2 == 0 && bb % 2 == 0 {
						mv := aa*16*32 + bb*32 - (aa*64*8 + bb*64)
						if mv > 32*32*32 {
							fmt.Println("mvmvmvmv = ",mv)
						}
						rotations = append(rotations, mv)
	...
	case "Conv_xc": // we assume manual padding using kp_wid
		if boot {
		...
		if boot {
			...
			for i, elt := range cont.in_wids {
				...
				for ul := 0; ul < iter; ul++ {
					...
					cont.ext_idx[elt][ul] = gen_keep_vec(cont.N/2, cont.in_wids[i], cont.kp_wids[i], ul)
					//println("elt elt elte ltelt el teltelt? ", elt)
					if elt == 8 {
						cont.ext_idx[elt][ul] = gen_keep_vec_sparse(cont.N/2, cont.in_wids[i], cont.kp_wids[i], 4)	
					}
					if elt == 16 {
						cont.ext_idx[elt][ul] = gen_keep_vec_sparse(cont.N/2, cont.in_wids[i], cont.kp_wids[i], 2)	
					}
			...
			pos := 0
			log_sparse := 1
			cont.m_idx[elt][pos], cont.r_idx[elt][pos] = gen_comprs_sparse_xuchao(cont.N/2, elt, cont.kp_wids[1], log_sparse, 0, pos)
			//cont.m_idx_l[elt][pos], cont.m_idx_l[elt][pos] = gen_comprs_sparse_xuchao(cont.N/2, elt, cont.kp_wids[1], log_sparse, 1, pos)
			...
			cont.m_idx[elt][pos], cont.r_idx[elt][pos] = gen_comprs_sparse_xuchao2(cont.N/2, elt, cont.kp_wids[2], log_sparse, 0, pos)
	...
	case "Resnet_crop_sparse", "Resnet_crop_sparse_wide2", "Resnet_crop_sparse_wide3": // Generate ext_idx for extracting valid values from conv with "same" padding
		...
		fmt.Println("m_idx m_idx m_idx",elt, cont.kp_wids[1], log_sparse, 0, pos)
		...
		//cont.m_idx[elt][pos], cont.r_idx[elt][pos] = gen_comprs_sparse_multi(cont.N/2, elt, cont.kp_wids[2], log_sparse, 0, pos)
	...
	for i := 0; i < 65536/2; i += 1 {
		if i % 4 == 0 {
			ori := reverseBits(uint32(i), 15)
			end := reverseBits(uint32(i+1), 15)
			rot := (ori - end + 16384*2 ) % (16384*2)
			rotations = append(rotations, (int)(rot))
		}
	...
	*/
	...
	if boot {
		...
		if (kind == "Resnet_crop_sparse") || (kind == "Resnet_crop_sparse_wide2") || (kind == "Resnet_crop_sparse_wide3") || (kind == "Imagenet_sparse")  || (kind == "Conv") {
		...
		} else if (kind == "Resnet_crop_sparse") || (kind == "Resnet_crop_sparse_wide2") || (kind == "Resnet_crop_sparse_wide3") || (kind == "Imagenet_sparse") || (kind == "Conv") {
			btpParams.LogSlots = btpParams.LogN - 1
...
func main() {
	...
	*/
	...
	//split, _ := strconv.Atoi(os.Args[5])
	...
	if resnet {
		...
		debug := true // if turned on, it shows all intermediate input
		if wide_case == 1 {
			// test with small inputs
			//for b := 0; b < 512; b++ {
			//testResNet_xuchao(0, test_num, ker_wid, depth, split, debug, cf100)
			//}
			//testResNet_crop_sparse(0, test_num, ker_wid, depth, debug, cf100)
			testResNet_crop_sparse_multipic(0, test_num, ker_wid, depth, debug, cf100)
			//testResNet_crop_sparse_multipic_BL(0, 4, ker_wid, depth, debug, cf100)
		...
		//fmt.Println("Their start.")
		//testConv_in(batchs[i_batch], widths[i_batch], ker_wid, num_tests, boot)
		
		//fmt.Println("testConv_BL_in start.")
		//testConv_BL_in(4, 32, ker_wid, num_tests, boot)
		testConv_BL_in(batchs[i_batch], widths[i_batch], ker_wid, num_tests, boot)
		
		//fmt.Println("xuchao split coeff start.")
		//testConv_in_xuchao(batchs[i_batch], widths[i_batch], ker_wid, num_tests, split, boot)
		
		//fmt.Println("xuchao split one by one start.")
		//testConv_BL_one(batchs[i_batch], widths[i_batch], ker_wid, num_tests, boot)
	}
...
func printDebugCfs_xuchao(params ckks.Parameters, ciphertext *ckks.Ciphertext, valuesWant []float64, decryptor ckks.Decryptor, encoder ckks.Encoder,name string) (valuesTest []float64) {
	total_size := make([]int, 15)
	...
	fmt.Println("len val Want:", len(valuesWant))
	fmt.Println("params.Slots():", params.Slots())
	fmt.Println("pvaluesTest_pre:", len(valuesTest_pre))
	fmt.Println("len val Test:", len(valuesTest))
	
	//writeTxt(name + "_valuesWant",valuesWant)
	//writeTxt(name + "_valuesTest",valuesTest)
...
func printDebugCfsPlain(valuesTest, valuesWant []float64) {
	total_size := make([]int, len(valuesWant))
	count := 0
	total := 0.0
	fmt.Printf("ValuesTest:")
	//for i := range total_size {
	...
	for i := range total_size {
		if count < 20 {
			if valuesWant[i] != 0 {
				fmt.Printf("valuesTest = %6.10f, valuesWant = %6.10f delta = %6.10f\n", valuesTest[i],valuesWant[i],valuesTest[i]-valuesWant[i])
				total = total + math.Abs(valuesTest[i] - valuesWant[i])
				count++ 
				}
		}
		
		//if valuesTest[i]-valuesWant[i] > 0.02 || valuesTest[i]-valuesWant[i] < -0.02 {
		//	fmt.Printf("wrong !!!! valuesTest = %6.10f, valuesWant = %6.10f delta = %6.10f\n", valuesTest[i],valuesWant[i],valuesTest[i]-valuesWant[i])
		//}
	...
	fmt.Println("Abs total = ", total," total/total_size = ",total/float64(len(valuesWant)))
...
func printDebug(log_sparse int, params ckks.Parameters, ciphertext *ckks.Ciphertext, valuesWant []complex128, decryptor ckks.Decryptor, encoder ckks.Encoder) (valuesTest []complex128) {
	...
	fmt.Printf("ValuesWant:")
	for i := range total_size {
		fmt.Println("%6.5f, %6.5f", real(valuesWant[i]), real(valuesTest[i]))
	}
	fmt.Printf("... \n")
...
// vec = arrgvec with batch batches, each batch is sqr-sized
// print (i,j)-th position in [batches], only shows (show, show) entries show = 0 : print all
func prt_mat_norm_xuchao(vec []float64, batch, norm, show int, name string, half bool) {
	mat_size := len(vec) / batch
	row := int(math.Sqrt(float64(mat_size)))
	if half {
		row = row / 2
	...
	tmp := make([]float64, batch/norm)
	res := make([]float64, batch/norm * len(vec))
	j, k := 1, 1
	for i := 0; i < len(vec); i += batch {
		if (show == 0) || (((j <= show) || ((j > row-show) && (j <= row))) && ((k <= show) || ((k > row-show) && (k <= row)))) {
			fmt.Printf("(%d, %d): ", j, k)
...
func readTxt(name_file string, size int) (input []float64) {
	...
	file.Close()
	// fmt.Print(input)
	//fmt.Print("readTxt = ",len(input) , size)
...
func prep_Input(input []float64, raw_in_wid, in_wid, N, norm int, trans, printResult bool) (out []float64) {
	...
	if trans {
		...
		for i := 0; i < in_wid; i++ {
			for j := 0; j < in_wid; j++ {
				for b := 0; b < batch/norm; b++ {					
					if (i < raw_in_wid) && (j < raw_in_wid) {
						out[i*in_wid*batch+j*batch+b*norm] = input[k] // xuchao 
						//out[i*in_wid*batch+j*batch+b*norm] = 1.0
						k++
					}
				}
			}
	...
	//fmt.Println("vprep_Input = ", len(out))
...
func prep_Input_xuchao(input []float64, split, raw_in_wid, in_wid, N, norm int, trans, printResult bool) (out1 [][]float64) {
	out := make([]float64, N)
	batch := N / (in_wid * in_wid) * split
	
	out1 = make([][]float64, split)
	for i := 0; i < split; i++ {
		out1[i] = make([]float64, N)
	}
			...
			for j := 0; j < in_wid; j++ {
				for b := 0; b < batch/norm; b++ {					
					if (i < raw_in_wid) && (j < raw_in_wid) {
						...
						idx := (int)(b/delta)
						out1[idx][i*in_wid*(batch/split)+j*(batch/split)+(b%delta)*norm] = input[k]
						//out1[idx][i*in_wid*(batch/split)+j*(batch/split)+(b%delta)*norm] = 1.0			
						k++
					}
				}
			}
