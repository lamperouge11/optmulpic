...
func gen_comprs_sparse_xuchao(vec_size, in_wid, kp_wid, log_sparse, ul, pos int) (m_idx, r_idx map[int][]int) {
	m_idx = make(map[int][]int)
	r_idx = make(map[int][]int)
	...
	//fmt.Println("aaaaaaaaaaaaaaaaaaaaa ul", ul)		
	if log_sparse != 0 {
		if pos != 0 {
			panic("No pos != 0 cases for log_sparse != 0")
		}
		
		for j := 0; j < in_wid / 2; j++ { // kinds of mov depends on j			
			//tmp := make([]int, vec_size)
			//mk := []int{}			
			for i := 0; i < in_wid; i++ {	
				//fmt.Println("(ul * in_wid / 2 + j)/2 ul", (ul * in_wid / 2 + j)/2)				
				if j % 2 == 0 && i % 2 == 0 && (i / 2 < kp_wid) {
					tmp := make([]int, vec_size)				
					idx := reverseBits(uint32(j * in_wid + i), 9) 
												
					//mk = append(mk,(idx))							
					tmp[idx] = 1
															
					idx_move := j * in_wid + i * 2					
					idx_ori := j * in_wid + i
										
					ac := reverseBits(uint32(idx_move), 9)
					ab := reverseBits(uint32(idx_ori), 9)
					
					rot := ((int)(ab) + 512 - (int)(ac)) % 512 
					//fmt.Println("rot rot rot rot aaaaaaaa ",len(m_idx[rot]))
			...
			//if j == 0 {
			//	for i := 0; i < len(mk); i++ {
			//		fmt.Println("pdlvde j = ",j, int(mk[i]/8),len(mk),batch)			
			//	}
			//}
...
func gen_comprs_sparse_xuchao2(vec_size, in_wid, kp_wid, log_sparse, ul, pos int) (m_idx, r_idx map[int][]int) {
	m_idx = make(map[int][]int)
	r_idx = make(map[int][]int)
	//batch := 2 * vec_size / (in_wid * in_wid * (1 << log_sparse))
	// if kp_wid < in_wid/2 {
	...
	if log_sparse != 0 {
		...
		for j := 0; j < in_wid / 2; j++ { // kinds of mov depends on j			
			...
			for i := 0; i < in_wid; i++ {	
				...
				if j % 2 == 0 && i % 2 == 0 && (i / 2 < kp_wid) {
					tmp := make([]int, vec_size)				
					idx := reverseBits(uint32(4 * j * in_wid + 4 * i), 9) 
												
					//mk = append(mk,(idx))							
					tmp[idx] = 1
															
					idx_move := j * in_wid * 4 + i * 4 * 2				
					idx_ori := (j * in_wid + i ) * 4
...
func gen_comprs_sparse_multi(vec_size, in_wid, kp_wid, log_sparse, ul, pos int) (m_idx, r_idx map[int][]int) {
	m_idx = make(map[int][]int)
	r_idx = make(map[int][]int)
	//batch := 2 * vec_size / (in_wid * in_wid * (1 << log_sparse))
	// if kp_wid < in_wid/2 {
	...
	fmt.Println("aaaaaaaaaaaaaaaaaaaaa in_wid", in_wid, kp_wid)		
	if true {
		if pos != 0 {
			panic("No pos != 0 cases for log_sparse != 0")
		}
		...
		for j := 0; j < in_wid / 2; j++ { // kinds of mov depends on j			
			...
			for i := 0; i < in_wid; i++ {	
				...
				if j % 2 == 0 && (i / 2 < kp_wid) {
					tmp := make([]int, vec_size)				
					idx := reverseBits(uint32(4 * j * in_wid + 4 * i), 15) 
												
					//mk = append(mk,(idx))							
					tmp[idx] = 1
															
					idx_move := j * in_wid * 8 + i * 16				
					idx_ori := (j * in_wid + i ) * 4
										
					ac := reverseBits(uint32(idx_move), 15)
					ab := reverseBits(uint32(idx_ori), 15)
					
					rot := ((int)(ab) + 32768 - (int)(ac)) % 32768
...
func gen_comprs_sparse(vec_size, in_wid, kp_wid, log_sparse, ul, pos int) (m_idx, r_idx map[int][]int) {
	...
	if log_sparse != 0 {
		...
		for j := 0; j < min_wid; j++ { // kinds of mov depends on j
		//for j := 0; j < 1; j++ { // kinds of mov depends on j
			tmp := make([]int, vec_size)
			mk := []int{}
			for b := 0; b < batch; b++ {
				...
				//for i := 0; i < 1; i++ {
					for k := 0; k < 2; k++ {
					//for k := 0; k < 1; k++ {
						///fmt.Println("reverseBits",uint32(j), log_in_wid-1,reverseBits(uint32(j), log_in_wid-1),uint32(kp_wid))
			...
			if log_sparse == 1 && j == 0 {
				sort.Ints(mk)
				//for i := 0; i < len(mk); i++ {
				//	fmt.Println("ascadd j = ",j, mk[i],int(mk[i]/8),len(mk),batch)			
				//}
			}
